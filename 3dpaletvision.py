...
def draw_pallet_layout(layers):
    """
    Визуализирует 2D-раскладку каждого слоя коробов на паллете и создает ZIP-архив.
    
    Args:
        layers (list): Список слоев с данными о коробах.
    
    Returns:
        BytesIO: Буфер с ZIP-архивом всех изображений.
    """
    if not layers:
    ...
    with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
        ...
        for layer_idx, layer in enumerate(layers):
            ...
            if boxes_per_layer > 1000:
                st.warning(f"Слой {layer_idx + 1}: Слишком много коробов для отображения. Пропущен.")
                continue
            ...
            ax.set_title(f"Слой {layer_idx + 1}: Тип короба {layer['box_index'] + 1} ({boxes_in_length}×{boxes_in_width})")
            ...
            zip_file.writestr(f"layer_{layer_idx + 1}_type_{layer['box_index'] + 1}.png", img_buffer.getvalue())
...
# Streamlit интерфейс
# Добавление логотипа
logo_path = "Rubineta_logo.jpg"
if os.path.exists(logo_path):
    st.image(logo_path, width=200)
else:
    st.warning("Файл логотипа (Rubineta_logo.jpg) не найден. Убедитесь, что он находится в той же директории, что и скрипт.")
...
if calculate:
    ...
    if result["fit_count"] == 0:
        ...
        if zip_buffer:
            st.download_button(
                label="Скачать визуализацию всех слоев (ZIP)",
                data=zip_buffer,
                file_name="pallet_layers.zip",
                mime="application/zip"
            )

        # Скачивание первого слоя как PNG (для обратной совместимости)
        if result['layers']:
            ...
            if boxes_per_layer <= 1000:
                ...
                ax.set_xlim(0, PALLET_LENGTH)
                ax.set_ylim(0, PALLET_WIDTH)
                ax.set_title(f"Слой 1: Тип короба {layer['box_index'] + 1} ({boxes_in_length}×{boxes_in_width})")
                ...
                st.download_button(
                    label="Скачать визуализацию первого слоя (PNG)",
                    data=img_buffer,
